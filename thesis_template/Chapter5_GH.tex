...
Sequencing quality of reads was checked with fastQC (\url{https://www.bioinformatics.babraham.ac.uk/projects/fastqc/}). Duplicate reads were marked with picard-tools \texttt{MarkDuplicates} \cite{Picard2018toolkit}. Libraries from the same samples were merged using \texttt{samtools merge} \cite{Li2009}, and duplicates were marked again as described above to tag duplicate reads of the same libraries among sequencing lanes. Genotypes and genotype likelihoods were called using the atlas pipeline \cite{Link2017}. First, aligned reads from paired-end sequenced read groups were merged to avoid double-use of bases in the overlapping part. PMD patterns were estimated for each read group with atlas \texttt{task=PMD}, providing the reference with \texttt{ref=} and enabling the option \texttt{filterSoftClips} to remove reads with soft-clipping. 

Again using atlas \cite{Link2017}, base-quality recalibration parameters were obtained (short recal-parameters) from highly conserved regions. A \texttt{.bam} file with corrected recalibration and \texttt{pmd} parameters was created with atlas \texttt{BAMUpdateQualities}, providing PMD and recal-parameters and applying soft-clip filter (\texttt{pmdFile}, \texttt{withPMD}, \texttt{recal}, \texttt{filterSoftClips}).
