...
Phasing and imputation ideally require a reference panel of high-coverage present-day individuals. I used the 1000 Genomes dataset (re-sequenced to 30x coverage), containing 3202 individuals from 26 worldwide populations \cite{byrska2021high}. A description of the processing of this reference dataset can be found in appendix ...
...
I used rtg-tools-3.11 \cite{cleary2014joint} and the \texttt{vcfeval} task to estimate the sensitivity and specificity of variant discovery in the downsampled individuals. Here, I define the `baseline' (i.e.\ truth) to be the genotype calls in the full coverage individual and the `calls' as the genotype calls in the downsampled individual. Sensitivity and precision can be calculated as: 
...
A ``variant'' is considered to be a SNP with a genotype that is either \texttt{0/1} or \texttt{1/1}, with $V_{baseline}$ and $V_{call}$ the number of variants called in the full coverage and downsampled genomes, respectively. I consider false negatives (FN) to be cases where a variant is called in the full coverage genome but not in the downsampled genome. I consider false positives (FP) to be cases where a variant is called in the downsampled genome but not in the full-coverage genome.
...
\begin{figure}[htp]
    ...
    \caption{TVD (metric of copyvector dissimilarity between two individuals) between each downsampled ancient individual and a flat copyvector. Flat copyvector equivalent to a vector of length $N$ where each element = $1/N$.{\color{red}[LBK SEEMS TO BE SIMLAR TO THE PRIOR FOR THE FULL-COVERAGE, ODDLY? WHY IS THIS -- ARE THERE NO OTHER EURO/ANATOLIA FARMERS IN THE REFERENCE? SHOULD EXPLAIN THIS BRIEFLY.]}}
...
\begin{figure}[htp]
    ...
    \caption{Principle component analysis (PCA) of downsampled, full coverage and reference ancient individuals generated from the linked{\color{red}[HAVE YOU DEFINED THIS?]} chunklengths matrix. Full coverage and downsampled genomes of the same individual are coloured the same. Reference individuals are grouped into populations plotted as the mean principle components for all individuals within the population. Numbers in labels correspond to the number of individuals within the reference population. 0.1x samples have red border for clariyu.}
